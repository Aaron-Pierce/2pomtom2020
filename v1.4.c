...
int main()
{
   ...
   /* int startLfSec = seconds();
    
    printf("\n begin bounce lf\n");
    while(seconds() - startLfSec < 10){
    ...
    int duaLength = 8;
    drive_until_analog_advanced(-1200, rlf, blackThreshold - 500, duaLength * 2, 4000);
    if(seconds() - startDuaSeconds < duaLength){
        printf("\n\n*****************************DUA BROKEN****************************\n");
        ...
        mav(rm, 500);
        msleep(250);
        while(analog(rlf) > blackThreshold){ //be more tolerant of black
            mav(rm, 500);
        }
        mav(rm, 0);
        msleep(50);
        }
        dwg(-1200, (duaLength*1000) - (timeElapsed*1000));
    }
