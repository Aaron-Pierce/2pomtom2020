...
int rampApproach(int duaLength, int startDuaSeconds){
    ...
    if(duaac(-1200, llf, rlf, blackOnBrownThreshold, duaLength - (seconds() - (double)startDuaSeconds)*1000)){
        printf("\n\n*****************************DUA BROKEN****************************\n");
        ...
        rampApproach(duaLength, startDuaSeconds);
...
*/
    ...
    int desired = get_servo_position(reg) + 450;
    printf("\n desired pos %i\n", desired);
    ...
    twg(19 - get_absolute_theta());

    dwg(-1200, 300);

    moveRel(-(get_servo_position(reg) - 775));
    mav(0, 600);
    msleep(5000);
    mav(0, -600);
    msleep(2000);
