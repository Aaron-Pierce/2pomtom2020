...
const int backLFThreshold = 3800;
...
int rampApproach(int duaLength, int startDuaSeconds, int moves){
    ...
    if(duaac(-1200, llf, rlf, blackThreshold, duaLength - (seconds() - (double)startDuaSeconds)*1000)){
        ...
        while(analog(sensorPort) > blackThreshold){ //be more tolerant of black
            mav(portToMove, vel);
        }
        freeze(portToMove);
        mav(portToMove, 0);
...
int main()
{
    ...
    */
    ...
    while(seconds() - paraAlignSec < 7){
        ...
        freeze(lm);
        while(analog(blf) < backLFThreshold){
            mav(rm, -1400);
        }
        freeze(rm);
    }
...
*/
    ...
    while(accel_y() < -150 && seconds() - beginHASec < 4){
        printf("HACC: %d\n", accel_y());
        mav(lm, -1500);
        mav(rm, -1500);
    }
